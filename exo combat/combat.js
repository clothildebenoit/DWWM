...
function info(info) {
    colorLog("Nom : " + info.GetNom() + info._type + "\n Vie : " + info.GetVie() + "\n Attaque : " + info.GetAttaque() + "\n Defense : " + info.GetDefense(), "info");
}
...
class person {
    constructor(unnom) {
        ...
        this.attaquer = function (defenseur) {
            colorLog("Round : " + cptRound + "\nNouvelle attaque de " + _nom + this._type + " sur " + defenseur.GetNom() + defenseur._type + " !!", "warning");
...
class match {
    constructor() {
        ...
        this.CreerPersonnages = function () {
            ...
            nbrJoueur = parseInt(prompt("Choisissez le nombre de personnages que vous souhaitez creer : "));
            if (nbrJoueur % 2 != 0) {
        ...
        function win(players) {
            if (players.some(player => player._type === " qui est un CRS")) {
                colorLog("Les CRS sont les vainqueurs ! Au round : " + cptRound, "vainqueur");
                alert("Les CRS sont les vainqueurs ! Au round : " + cptRound, "vainqueur");
            } else if
                (players.some(player => player._type === " qui est un Gilet Jaune")) {
                colorLog("Les Gilets Jaunes sont les vainqueurs ! Au round : " + cptRound, "vainqueur");
                alert("Les Gilets Jaunes sont les vainqueurs ! Au round : " + cptRound, "vainqueur");
            }
            
            for (let k = 0; k < players.length; k++) {
                console.log("Le "+ players[k].GetNom() + " est toujours en vie! Bravo !");
           }
        ...
        this.RunMatch = function () {
            ...
            while ((players.some(player => player._type === " qui est un CRS")) && (players.some(player => player._type === " qui est un Gilet Jaune"))) {
